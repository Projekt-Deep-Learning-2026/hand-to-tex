...
class TestHMECollateFunction:
    def test_pads_features_to_max_length(self, vocab):
        ...
        batch = [
            (torch.ones(2, 10), torch.tensor([1, 2], dtype=torch.long)),
            (2 * torch.ones(5, 10), torch.tensor([3], dtype=torch.long)),
        ]
        ...
        assert torch.allclose(padded_ft[1], 2 * torch.ones(5, 10))

    def test_pads_tokens_with_pad_idx(self, vocab):
        collate = HMECollateFunction(vocab)
        batch = [
            (torch.ones(2, 10), torch.tensor([1, 2, 3], dtype=torch.long)),
            (torch.ones(2, 10), torch.tensor([4], dtype=torch.long)),
        ]
    ...
    def test_single_sample_batch_returns_correct_shapes(self, vocab):
        collate = HMECollateFunction(vocab)
        batch = [
            (torch.randn(7, 10), torch.tensor([5, 6, 7], dtype=torch.long)),
        ]
    ...
    def test_equal_length_sequences_no_padding_needed(self, vocab):
        collate = HMECollateFunction(vocab)
        batch = [
            (torch.ones(3, 10), torch.tensor([1, 2], dtype=torch.long)),
            (2 * torch.ones(3, 10), torch.tensor([3, 4], dtype=torch.long)),
        ]

        padded_ft, ft_lengths, padded_ts, ts_lengths = collate(batch)

        assert padded_ft.shape == (2, 3, 10)
        assert padded_ts.shape == (2, 2)
        # Original values preserved
        assert torch.allclose(padded_ft[0], torch.ones(3, 10))
        assert torch.allclose(padded_ft[1], 2 * torch.ones(3, 10))

    def test_output_dtypes(self, vocab):
        collate = HMECollateFunction(vocab)
        batch = [
            (torch.ones(2, 10, dtype=torch.float32), torch.tensor([1], dtype=torch.long)),
        ]
